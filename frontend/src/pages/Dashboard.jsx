...
export default function Dashboard() {
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "#000",
      backgroundImage: "radial-gradient(circle at 50% 0%, rgba(0, 229, 255, 0.1) 0%, transparent 60%)",
      color: "#fff",
        ...
        {!hasAccess ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ textAlign: "center", paddingTop: 40, maxWidth: 1200, margin: "0 auto" }}>
            ...
            {waitlist?.in_waitlist ? (
              <>
                <div style={{ fontSize: 14, textTransform: "uppercase", letterSpacing: "0.2em", color: "#00E5FF", marginBottom: 20 }}>Your Waitlist Position</div>
                <div style={{ fontSize: "140px", fontWeight: 200, lineHeight: 1, letterSpacing: "-0.04em", textShadow: "0 0 80px rgba(0, 229, 255, 0.4)" }}>
                  #{waitlist.position || "—"}
                </div>
              ...
              <div style={{ maxWidth: 400, margin: "0 auto" }}>
                ...
                />
                <button onClick={joinWaitlist} disabled={waitlistLoading} style={{ width: "100%", padding: 16, background: "#00E5FF", color: "#000", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 600, cursor: "pointer" }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} style={{ maxWidth: 1600, margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(450px, 1fr))", gap: 60, marginBottom: 100 }}>
              ...
              <div>
                ...
                <div style={{ background: "rgba(0,0,0,0.5)", border: "1px solid rgba(0, 229, 255, 0.2)", borderRadius: 16, padding: 24, minHeight: 280, fontFamily: "'Source Code Pro', monospace", fontSize: 13, color: "#00E5FF", boxShadow: "inset 0 0 40px rgba(0,229,255,0.05)" }}>
                  ...
                  <div style={{ marginTop: 24, display: "flex", alignItems: "center", gap: 8, opacity: 0.5 }}>
                    <div style={{ width: 8, height: 16, background: "#00E5FF", animation: "blink 1s infinite" }} />
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                ...
                {/* Mock Emotion Map */}
                <div style={{ gridColumn: "span 2", background: "linear-gradient(135deg, rgba(157, 76, 221, 0.1) 0%, transparent 100%)", border: "1px solid rgba(157, 76, 221, 0.2)", borderRadius: 16, padding: 24 }}>
                  <div style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: "0.1em", color: "#9D4CDD", marginBottom: 16, display: "flex", justifyContent: "space-between" }}>
                    <span>Stress / Emotion Map</span>
                    <span style={{ fontSize: 11, background: "rgba(157,76,221,0.2)", padding: "2px 8px", borderRadius: 10 }}>Live</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "flex-end", height: 80, gap: 4 }}>
                    {/* Mock Graph Bars */}
                    {[40, 60, 30, 80, 50, 20, 90, 45, 60, 20, 10, 30].map((h, i) => (
                      <div key={i} style={{ flex: 1, background: `rgba(157, 76, 221, ${h/100})`, height: `${h}%`, borderRadius: "4px 4px 0 0", transition: "height 1s" }} />
                    ))}
                  </div>
...
function FeatureTeaser({ icon, title, desc }) {
  return (
    <div style={{ textAlign: "left", padding: 30, borderRadius: 24, border: "1px solid rgba(255,255,255,0.1)" }}>
      <div style={{ color: "#00E5FF", marginBottom: 20 }}>{icon}</div>
